...
#include "../basicfundll/AES/ocx_aes.h" //控件的AES加解密算法
#include "../basicfundll/jpegdeal/jpegdeal.h" //libjpeg库
...
//从jobj的fieldName(jstring)类型，申请对应的char*内存并存放，需要自己释放内存
...
//	env->SetObjectField(obj_IMSGotUserInfo, id_fieldID, ContentArray);
    ...
    {
        ZX_free(pResult->cPhotoFile);//别忘记了释放(在解包的时候分配的内存)
...
//    env->SetIntField(obj_GroupInfo, id_fieldID, pGroupInfo->iNum);
    env->SetIntField(obj_GroupInfo, id_fieldID, count); //改为分批上报
...
//    jobjectArray array_group = env->NewObjectArray(pGroupInfo->iNum, class_FireAllGroup, NULL);
    jobjectArray array_group = env->NewObjectArray(count, class_FireAllGroup, NULL); //改为分批上报
...
void java_FireIMSGotAddressList(int iNum, short far* pNotify)
{
    ...
    for(i = 0; i < iNum; i++)
    {
        ...
        if(g_WriteLogType == 2)//2013.06.05添加测试日志用于定位问题
...
void java_FireIMSGotOneListInfo(short far* pListInfo, int iReturnCode,LPCTSTR pURI)
{
    ...
    if(pResult == NULL)//联系人为空直接返回不需要上报
    {
        if(g_WriteLogType == 2)
...
void java_FireIMSDispUICmdResult(int iType, short* pPara)
{
    ...
    if (iType == 5)
    {
        ...
        if((pImPreStructInfo->iOperType == 1) ||
           ...
           (pImPreStructInfo->iOperType == 4))//表示查询消息类业务相关选项设置结果
        {
...
#if 0
        jstr = env->NewStringUTF(pMemberInfo);//导致登录有的时候core的问题
...
static jboolean jni_launcherThreads(JNIEnv* env, jclass cls, jstring jstrcfgName)
{
    ...
    SetCfgfileName(cfgName);//从界面得到gcCfgFile路径了
#if 0
    if(WRITELOG == 1)
        SetLogFileName();//设置日志初始化路径
#endif
    LoadLogConfig();
    ...
    StartAllAgent();//可以zai次之前初始化日志路径,不过关于so加载部分的日志不能打印了。
...
{//iOperType: 0--根据etag增量查询, 1--到服务器全量下载
...
static jobject jni_bSyncGetGroupMembers(JNIEnv* env, jclass cls, jstring jstr_Identifier)
{
    ...
    if(!ReadLocalGroupDataFile(filename, &gMsgBuf, &groupData, &memberList))
    {
        ...
        //通知softagent主动去获取
        if(pGroup)//pGroup没有找到的情况，即使调用方法也会被拒
    ...
    else
    {
        ...
        {//本地etag和登陆时获取个人信息返回的不相同，那么通知softagent主动去获取
...
//add by zhanghe 2011-11-15 处理请求订阅本人信息的应答方
...
static jboolean jni_bIMSLoadAddrList(JNIEnv* env, jclass cls, jint iType, jobject PersonListInfoList, jint iNum, jint iPage)
{
    ...
    if(iType == 0)
    {
        ...
        size = (env)->CallIntMethod(PersonListInfoList, sizeMethodID);//该size应该和iNum一样
    }
    if(size > 0)//如果size和iNum值不一样，就应该以size为准
    {
...
{
    char cReqMemberName[256 + 1] = {0}; //申请人的URI，格式为：sip:帐号@域名，最长64字节
    char cReqURI[256 + 1] = {0}; //申请人的URI，格式为：sip:帐号@域名，最长64字节
    char cReturn1[256 + 1] = {0}; //从事件IMSReqJoinMyPublicGroup中得到的字符串, 实际为 公共群组的URI
    char cReturn2[128 + 1] = {0}; //从事件IMSReqJoinMyPublicGroup中得到的字符串, 实际为 公共群组文档的URL
    ...
    jstring jstr_pDisplayName, jstring jstr_pShareUserURI, jstring jstr_pShareUserName)
{
    ...
    char cDisplayName[64 + 1] = {0};
    char cShareUserURI[64+1] = {0};//二维码分享者sip账号
	char cShareUserName[128+1] = {0};//二维码分享者姓名
...
typedef struct AVTEST_METHOD {
	int		iOperateType;	//0-停止; 1-开始
	int		iAudioType;	//0-无音频; 1-G711a; 2-G711u; 3-G729AB; 4-SILK
	int		iVideoType;//0-无视频; 1-H264; 2-H263
	char	cRemoteIPAddress[16+1];//远端IP地址
	char	cLocalIPAddress[16+1];//本地IP地址
	long	lRemoteAudioPort;//远端IP的音频端口号
	long	lRemoteVideoPort;//远端IP的音频端口号
	long	lLocalAudioPort;//本端IP的音频端口号
	long	lLocalVideoPort;//本端IP的视频端口号
}AVTEST_METHOD;
...
static void jni_AVTest(JNIEnv* env, jclass cls, jobject jobj_AVTestMethodObject)
{
	...
	if (AVTestMethod.iOperateType == AVTEST_OPERATE_STOP)//停止
	{
...
//			MediaEngin_AudioEnd();
	...
	else if(AVTestMethod.iOperateType == AVTEST_OPERATE_START)//开始
	{
		...
		else
		{
			...
			WORD m_wAudioMillSecsPerFrame = 20;		//默认按G711a参数填写
			unsigned int m_AuidoRtpPT = G711ALAW;	//默认按G711a参数填写
		...
		{
		case AVTEST_VIDEOTYPE_NONE:			//无视频
			{
			}
			break;
...
{//iType: 1-查询自己关注的公众号, 2--关注公众号 3--取消关注公众号
    char cPubAccId[40] = {0};
...
{//iType: 1-通过公众号名字模糊查询，2-通过公众号ID查询, 3-按照关注人数查询，4-按照创建时间查询
...
{//iType: 1—获取群二维码并更新二维码有效期，2-根据key获取json串
...
//    ZX_free(pXmlBody);//后续softagent用完后释放
    return bRet;
}
...
{//iOperType:操作类型,1-bind 2-unbind 3-查询bind关系
...
{/*iType: 1-查询设置了消息免扰的群、联系人、公众号列表  2-设置群免扰 3-取消设置群免扰  4-设置联系人免扰 5-取消设置联系人免扰
6-设置公众号免扰 7-取消设置公众号免扰*/
    char cURI[256+1] = {0};
    safeGetStringUTFChars(env, jstrURI, cURI, sizeof(cURI) - 1, NULL);
...
{//iOperType: 1-阅读回执上报  2-单聊阅读回执查询详情  3-群聊阅读回执查询详情
...
/*
pSessionID：标识本次上传文件的ID号,长度为32字节
pFilePathName：带完整路径的文件名,最长255字节
iOperType:  0-新增上传，1-断点续传，2-取消上传（pSessionID与请求pSessionID保持一致）
pFileID：断点续传（iOperType=1）需要填写服务器生成的文件ID，其他情况填""
*/
...
/*
pSessionID：标识本次请求的会话ID号,长度为32字节
pFilePathName：用于保存文件的带完整路径的文件名，最长255字节
iOperType:  0-新增下载，1-断点续传，2-取消下载（pSessionID与请求pSessionID保持一致）
pFileID：填写服务器生成的待下载文件ID
*/
...
/*
jstrSessionID:会话ID,响应时控件透传回UI,最大长度32字节
jstrRequestPath: 请求url的路径，如: http://IP:PORT/xxxxx中的"xxxxx"部分内容
jstrMethod: HTTP方法，如GET,POST等
jstrBody:HTTP的body内容，没有body时填写""
*/
...
static JNINativeMethod methods[] =
{
    ...
    //通过xcap协议栈发送http请求给其他的webservices
...
//getStringField函数中对空的指针应该做个保护(但是如果对象不空，只是对象的某部分内容空
//控件很难保护。所以界面传给控件的对象不能为空，对象的某部分内容如果没有，就要初始化""
//否则控件不找不到\0结束符。)
//控件也可以试着对某个对象如果为空也做保护。
//最好是界面做保护
char *getStringField(JNIEnv* env, jobject jobj, jclass jcs,  const char * fieldName, char *result, int ilen)
{
    const char *str = NULL;
